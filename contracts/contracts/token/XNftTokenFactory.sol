...
contract XNftTokenFactory is Ownable {

    event XNftTokenCreated(address owner, string name, string symbol, string contractURI, string tokenURIPrefix);
    ...
    function createXNftToken(
      string memory name,
      string memory symbol,
      string memory contractURI,
      string memory tokenURIPrefix
    ) external returns (address _token) {
        XNftToken token = new XNftToken(name, symbol, msg.sender, contractURI, tokenURIPrefix);
        ...
        emit XNftTokenCreated(msg.sender, name, symbol, contractURI, tokenURIPrefix);
    }
}
